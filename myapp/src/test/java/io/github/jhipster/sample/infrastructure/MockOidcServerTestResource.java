...
public class MockOidcServerTestResource implements QuarkusTestResourceLifecycleManager {
    ...
    public static String getAccessToken(String username, Set<String> groups) {
        var key = getJwk();
        return Jwt.preferredUserName(username).groups(groups).jws().keyId(key.getKeyId()).sign(key.getPrivateKey());
    }
    ...
    public Map<String, String> start() {
        ...
        server.start();
        WireMock.stubFor(
            get(urlEqualTo("/jhipster-mock-oidc/.well-known/openid-configuration")).willReturn(
                aResponse()
        ...
        WireMock.stubFor(
            get(urlEqualTo("/jwks")).willReturn(
                aResponse().withHeader("Content-Type", MediaType.APPLICATION_JSON).withBody(new JsonWebKeySet(getJwk()).toJson())
            )
        );

        WireMock.stubFor(
            get(urlEqualTo("/userinfo")).willReturn(
                aResponse().withHeader("Content-Type", MediaType.APPLICATION_JSON).withBody("{\"groups\":[\"ROLE_ADMIN\"]}")
            )
        );

        log.info("Mock OIDC server started: {}", server.baseUrl());
        return Map.of("quarkus.oidc.auth-server-url", server.baseUrl() + "/jhipster-mock-oidc");
    }
